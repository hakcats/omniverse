...
def push_audio_track(url, audio_data, samplerate, instance_name):
    ...
    with grpc.insecure_channel(url) as channel:
        stub = lipsync_grpc.Audio2FaceStub(channel)
        request = lipsync.PushAudioRequest()
    ...
    with grpc.insecure_channel(url) as channel:
        ...
        def make_generator():
            ...
            for i in range(len(audio_data) // chunk_size + 1):
                ...
                yield lipsync.PushAudioStreamRequest(audio_data=chunk.astype(np.float32).tobytes())
